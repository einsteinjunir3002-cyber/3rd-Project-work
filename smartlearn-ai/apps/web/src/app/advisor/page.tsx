...
async function getMentorshipRequests(token: string) {
  try {
    ...
    console.error("Failed to fetch advising requests:", error);
    return [];
  }
}
...
async function getMentors(token: string) {
  try {
    ...
    console.error("Failed to fetch advisors list:", error);
    return [];
  }
}
...
export default async function AdvisorPage() {
  ...
  const allowedRoles = ["CAREER_ADVISOR", "SYSTEM_ADMINISTRATOR"];
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-100 font-sans">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16">
            <div className="flex items-center">
              <h1 className="text-xl font-bold text-sky-400 tracking-tight">SmartLearn AI</h1>
              <span className="ml-4 px-3 py-1 bg-sky-900/50 text-sky-300 text-xs font-semibold rounded-full border border-sky-800">
                Career Advisor
              </span>
            </div>
            ...
            <div className="flex items-center space-x-6">
              ...
              <form action={async () => {
                ...
                <button type="submit" id="btn-advisor-signout" className="text-sm font-semibold text-rose-400 hover:text-rose-300 transition">
                  Sign Out
                </button>
              </form>
            </div>
      ...
      <main className="max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8 animate-fade-in">
        <div className="mb-10">
          <h2 className="text-3xl font-extrabold tracking-tight text-white">Career Advising Hub</h2>
          <p className="text-sm text-gray-400 mt-1">Configure advising credentials, areas of expertise, and manage student matching tickets.</p>
        </div>

        <AdvisorDashboardClient initialProfile={myProfile} initialRequests={requests} token={session.user.accessToken} />
      </main>
    </div>
  );
}
